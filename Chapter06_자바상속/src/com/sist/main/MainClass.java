 ...
 *     2) 재사용 기법
 *        = 반복 제거 
 *        = 상속(is-a) / 포함(has-a) => 클래스의 크기 비교
 *        = 이미 사용중에 있다 (신뢰성)
 *        => 재사용(상속, 포함) , 가독성 (메소드), 견고성(예외처리)
 *        => 기본문법 (8장) => 9자 : 조립식 (라이브러리 이용)
 *        => 프로그램 (네트워크 , 데이터베이스)
 *     
 *     3) 자바의 상속 (194p)
 ...
 *            = 한개만 상속이 가능 (단일 상속)
 *            = 형식
 *              A extends B => B가 가지고 있는 모든 것을 가져온다
 ...
 *                                             private
 *                             => 필요한 기능만 설정
 *                                멤버변수를 사용하려면 => private => protected 사용
 *                                                             =-----------
 *                             => 메소드의 기능을 변경해서 사용
 *                                => 오버라이딩
 ...
 *                                                 ----------------------------------
 *                                                 두개의 메모리 할당되기때문에 속도 느려지기때문!
 *                                => 스프링에서는 상속 개념이 거의 없다 (일반 클래스로 사용 => 처리)
 *                                                               -------- POJO방식
 *                               => 자바의 모든 클래스
 *                                       --------
 *                                       | 라이브러리 / 사용자정의클래스 => Object클래스 상속을 받는다
 ...
 *          지니뮤직 ===> 상속 ===> 멜론
 *          CGV    ===> 상속 ===> 롯데시네마 / 메가박스
 *          상속을 받는 경우 => 그대로 사용 (x) ==> 변경 / 다른 기능 추가
 *                                           -----------------
 *                                           오버라이딩 / 오버로딩 ==> 다형성
 *          *** 객체 지향의 3대 특성
