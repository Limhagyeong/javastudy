...
import java.util.*;
import java.io.*;
// 입출력 => IO => 속도 느려짐 ===> 메모리에서 제어 => 메모리 저장값을 IO를 이용해서 파일로 전송!
// 캡슐화
class Student
{
	private int hakbun;
...
class StudentManager
{
	...
	static 
	{
		...
		try
		{
			...
			for(String s:stds)
			{
				...
				ss.setHakbun(Integer.parseInt(st.nextToken())); // String => int
				ss.setName(data);
	...
	public void studentUpdate(int hakbun, Student ss)
	{
		...
		{
			Student s=list.get(i); // 인덱스 번호를 이용해서 학번을 찾는다
			if(s.getHakbun()==hakbun)
			{
				break; // break 되기 전까지 돌아간 수 => 인덱스번호 
			}
		}
		list.set(i, ss); // 수정
	...
	public void studentDelete(int hakbun)
	{
		...
		for(i=0;i<list.size();i++)
		{
			Student s=list.get(i); // 삭제 대상의 인덱스 번호
			if(s.getHakbun()==hakbun)
			{
				break; // i가 몇번인지 확인!
			}
	...
	public void save() // 수정, 삭제 내용 저장
	{
		...
		try
		{
			...
			for(Student s:list)
			{
				msg+=s.getHakbun()+"|"+s.getName()+"|"+s.getKor()+"|"
	...
	public void getRead() // 수정, 삭제된 내용 불러오기
	{
