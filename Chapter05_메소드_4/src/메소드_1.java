 ...
 *       ...
 *       return 값;
 *       => return 문장은 메소드의 종료 => 반드시 필요함
 *       => void 선언 시 return 생략할 수 있다
 *                     ----------
 *                     컴파일러에 의해 자동 추가된다
 *     }
 *     
 ...
 *         * void => 결과를 메소드에서 출력하겠다
 *       = 리턴형과 결과값은 가급적이면 동일한 데이터형으로 사용한다
 ...
 *       3. 특수문자 사용 가능 (_,$)
 *       4. 키워드는 사용할 수 없다
 *          ---- 변수명, 메소드명, 클래스명은 키워드가 아니다
 *       5. 공백 사용 불가
 *       6. 메소드명의 길이는 제한X (3~15자 이내가 적절)
 *          => 추가 : board_insert , boardInsert) 
 *       7. 약속
 *          소문자로 시작
 *          두개의 단어일 경우 두번째 단어의 첫글자는 대문자로 => 헝거리어식 표기법
