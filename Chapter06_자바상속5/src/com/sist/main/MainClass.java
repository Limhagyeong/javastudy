package com.sist.main;
/*
 *  객체지향의 3대 특성 => 문법사항이 아니다, 권장사항
 *  --------------
 *   class Window extends JFrame => 재사용 => 상속
 *    => 변경사항이 있는 경우
 *    class Window
 *    {
 *      JFrame f=new JFrame() => 재사용 => 포함 => 변경사항이 없다
 *    }
 *    => 웹 / 스프링 => 상속이 거의 없다 => 포함
 *    
 *    1. 캡슐화 : 데이터 보호 => 데이터를 감추고 메소드를 통해 접근하는 방식
 *    
 *               멤버변수 => private(은닉화) => 기능설정을 통해 접근
 *                                         --------
 *                                         멤버변수 : 저장/읽기
 *                                         ----------------
 *                                              |
 *                                            getter, setter
 *
 *    2. 상속/포함 : 기존의 클래스를 재사용하는 목적
 *       상속 
 *       ---
 *       1. 목적
 *          => 재사용 (신뢰성, 빠른 개발속도)
 *          => 유지보수를 쉽게 만든다
 *          => 중복 최대한 제거
 *          => 통일성이 있는 코드 작성 
 *          => 단점 : 느린 실행 속도 (게임 => 자바(X),C/C++(X),어셈블러)
 *                   ---------- 최적화 => exe (웹,모바일)
 *                                           J2EE J2ME => J2SE
 *          => 장점 : 적은 양의 코드로 새로운 클래스를 작성
 *             class JFrame => 1000줄 
 *             {
 *               ---
 *               ---
 *               ---
 *               ---
 *             }
 *             
 *             class A extends JFrame
 *             {
 *                위 1000줄을 상속
 *             }
 *             
 *             코드를 공통적으로 관리하기때문에 여러 코드를 추가, 변경이 용이하다
 *                                      ------------ -----------
 *                                      오버로딩        오버라이딩**
 *       
 *            **** 자바는 오버라이딩이다 => 변경
 *                                    ----
 *                                      | 프레임워크 (실제 소스)
 *                                     스프링 => 전자정부프레임워크(O)
 *                                           => 애니프레임워크
 *       2. 작성방법
 *         => 확장한다 => 기존의 클래스를 확장해서 새로운 클래스 제작
 *            extends
 *         => class parent
 *            class child extends parent
 *                        -------
 *         => 단점 => 단일 상속만 가능하다
 *       3. 상속의 특징
 *          1) 생성자, 초기화 블록은 상속 X
 *             상속 받은 클래스의 객체 생성 시에 => 상속 내린 클래스가 먼저 메모리 할당을 한다
 *             
 *             class A
 *             {
 *                int a,b;
 *                String s;
 *               ------------ 
 *                public A()
 *                {
 *                } // => 생략 (자동 추가)
 *             }
 *              
 *             class B extends A
 *             {
 *                int c,d;
 *                
 *                public B()
 *                {
 *                    super() => 상속받은 클르스의 생성자를 먼저 호출
 *                }
 *               ------------- 자동 추가
 *             }
 *             
 *             B b=new B();
 *             
 *             <Stack> => 메모리에서 처리 (할당, 해제) 
 *             --b--
 *             
 *             -----    <Heap> => 실제 데이터 저장 (프로그래머)
 *                     --------------------------
 *                         ----Super--------
 *                             ---a----
 *                             
 *                             -------
 *                             ---b----
 *                             
 *                             --------
 *                             ----s----
 *                             
 *                             --------
 *                         -----------------
 *                         
 *                         ------This-------
 *                             ----c------
 *                             
 *                             ----------
 *                             -----d-----
 *                             
 *                             ----------
 *                         ----------------
 *                     ---------------------------
 *        => 모든 클래스의 상위 클래스 (최상위 클래스)
 *           => Object
 *        => private => 상속은 가능  / 접근이 불가능
 *                                  --------- protected 
 *        => 상속 => 상위 클래스 => super
 *                  ---------
 *                  자신 클래스 => this
 *                  --------
 *        => 상속을 내리는 클래스 : 상속받은 클래스에 접근이 불가능하다
 *           상속을 받는 클래스 : 자신 + 상속 내리는 클래스까지 접근이 가능
 *                            this + super
 *                                      
 *    3. 다형성 => 필요시에 
 *        => 변경해야 된다 => 오버라이딩 
 *        => 추가해서 사용 => 오버로딩
 *  [접근지정어] 리턴형 메소드명(매개변수)
 *  --------- ----- ------ ------
 *  {
 *  
 *  }
 *  ------------------------------------------------------------
 *                 오버라이딩               오버로딩 => 기술면접 100%! => 면접 기술 : 정의를 내린 후 부연설명!
 *                                              => 객체지향프로그램에서 객체란? : 현실세계를 특성화 시킨것이다
 *                                                                                 ---- 추상화 => 단순화
 *                                              => 써본 경험 바탕으로 답변
 *  ------------------------------------------------------------
 *   상태             상속                  같은 클래스 안에서      
 *             => 상속 없이도 가능하다!
 *             => 익명의 클래스
 *   메소드명          동일                      동일
 *   매개변수          동일                갯수나 데이터형이 다르다
 *   리턴형            동일                     관계없음
 *   접근지정어   축소 불가, 확대 가능              관계없음
 *      private < default < protected < public
 *      
 *      => 예)
 *             void aaa()
 *             
 *             => protected void aaa()
 *                public void aaa()
 *             => 메소드는 다른 클래스와 연결할 때 많이 사용
 *                ----- public
 *      => modify : 기존의 기능 변경, new => 새로운 기능 추가
 *            |                    |
 *         오버라이딩                오버로딩 
 *  ------------------------------------------------------------
 *  상속 예외조건 => static : 공통으로 사용이 가능
 *                생성자 / 초기화 블록
 *                private : 상속은 된다 => but, 접근 불가!
 *  주의점)
 *        접근지정어 
 *        클래스 => [접근지정어] class 클래스명
 *                 public => 다른 메모리 할당
 *        메소드 => [접근지정어] 리턴형 메소드명(매개변수)
 *                 public 
 *        멤버변수 => [접근지정어] 데이터형 변수명
 *                 private / protected
 *        생성자 => [접근지정어] 클래스명()
 *                 public
 *        지역변수, 매개변수 : 접근지정어 사용 불가
 *                         static 사용 불가
 *                         => final만 사용 가능 ==> 상수형
 *        메소드
 *           멤버메소드 : static, 멤버변수를 사용할 수 있다
 *           static메소드 : static만 사용 가능하다
 *                만약 => 멤버변수, 멤버메소드를 사용하게되면
 *                       반드시 객체 생성 후 사용해야한다
 *                class A
 *                {
 *                   private int a;
 *                   static int b;
 *                   public void aaa()
 *                   {
 *                       a,b,bbb()
 *                   }
 *                   public static void bbb()
 *                   {
 *                       b는 사용 가능
 *                       a,aaa() => 사용 불가
 *                       
 *                       A aa=new A(); => 객체 생성 후
 *                       aa.a
 *                       aa.aaa() ==> 형식으로 사용 가능
 *                   }
 *                }
 *        초기화블록 : 자동 호출
 *           = 인스턴스 블록
 *             {
 *               static, 멤버변수를 초기화 
 *             }
 *           = static 블록
 *            static{
 *               static변수만 초기화 
 *            }
 *        변수에 대한 초기화
 *          1) 명시적 초기화 private int a=10;
 *          -------------
 *          2) 초기화 블록 : static변수 초기화
 *             => static은 생성자를 호출할 필요가 없다 => 자동 메모리 할당
 *          3) 생성자 : 인스턴스변수 초기화
 *             => 인스턴스는 반드시 생성자 호출해야 저장
 *             => 호출 형식 : new 생성자()
 *             => 시작과 동시에 처리 
 *                윈도우 => 화면 설계
 *                웹 => 자동 로그인, 쿠키 읽기
 *                일반 => 데이터 베이스 연결 , 서버연결 .. 
 *          ------------- 클래스 영역은 선언만 가능 => 구현 불가 => 2,3번 이용해서 구현!
 *            구현 => 연산처리 , 제어문, 파일읽기, 오라클 데이터 앍기 ... 
 */
class A
{
	 int a;

	public A() {
		super(); // Object
		
	}
}
class B extends A
{
	int c;

	public B() {
		super();
	}
}
public class MainClass {

	public static void main(String[] args) {
		// TODO Auto-generated method stub

	}

}
